...
async function analyzeWithStrategy(
  ...
  // ============================================================
  // FILTER 1A: RSI EXHAUSTION CHECK - CRITICAL NEW FILTER
  // ============================================================
  // Don't enter trades at extreme RSI levels (trend exhaustion)
  const currentRSI = indicatorValues.get("RSI") || 50;
  
  if (signalType === "short" && currentRSI < 35) {
    console.log(`❌ ${data.symbol}: RSI too low for SHORT (${currentRSI.toFixed(1)}) - Market oversold, high risk of bounce`);
    await logRejection('RSI exhaustion - oversold', {
      rsi: currentRSI,
      signalType: signalType,
      reason: 'Cannot SHORT in oversold conditions - high reversal risk'
    });
    return null;
  }
  
  if (signalType === "long" && currentRSI > 65) {
    console.log(`❌ ${data.symbol}: RSI too high for LONG (${currentRSI.toFixed(1)}) - Market overbought, high risk of reversal`);
    await logRejection('RSI exhaustion - overbought', {
      rsi: currentRSI,
      signalType: signalType,
      reason: 'Cannot LONG in overbought conditions - high reversal risk'
    });
    return null;
  }

  // ============================================================
  // FILTER 1B: MOMENTUM DECELERATION CHECK - NEW FILTER
  // ============================================================
  // Check if trend momentum is slowing down using EMA slope
  const emaSlope = calculateEMASlope(prices);
  
  if (signalType === "short" && emaSlope > -0.0005) {
    console.log(`❌ ${data.symbol}: Bearish momentum too weak (EMA slope: ${emaSlope.toFixed(6)}) - Trend may be exhausting`);
    await logRejection('Weak bearish momentum', {
      emaSlope: emaSlope,
      signalType: signalType,
      reason: 'Bearish trend losing momentum - late entry risk'
    });
    return null;
  }
  
  if (signalType === "long" && emaSlope < 0.0005) {
    console.log(`❌ ${data.symbol}: Bullish momentum too weak (EMA slope: ${emaSlope.toFixed(6)}) - Trend may be exhausting`);
    await logRejection('Weak bullish momentum', {
      emaSlope: emaSlope,
      signalType: signalType,
      reason: 'Bullish trend losing momentum - late entry risk'
    });
    return null;
  }

  // ============================================================
  // FILTER 1C: PRICE OVEREXTENSION CHECK - NEW FILTER
  // ============================================================
  // Don't enter if price is too far from EMA (overextended moves)
  const currentPrice = parseFloat(data.lastPrice);
  const ema20 = indicatorValues.get("EMA") || currentPrice;
  const priceDistanceFromEMA = ((currentPrice - ema20) / ema20) * 100;
  
  if (signalType === "short" && priceDistanceFromEMA < -2.5) {
    console.log(`❌ ${data.symbol}: Price too far below EMA for SHORT (${priceDistanceFromEMA.toFixed(2)}%) - Overextended, bounce risk`);
    await logRejection('Price overextended below EMA', {
      priceDistanceFromEMA: priceDistanceFromEMA,
      signalType: signalType,
      reason: 'Price too far below EMA - high probability of bounce'
    });
    return null;
  }
  
  if (signalType === "long" && priceDistanceFromEMA > 2.5) {
    console.log(`❌ ${data.symbol}: Price too far above EMA for LONG (${priceDistanceFromEMA.toFixed(2)}%) - Overextended, pullback risk`);
    await logRejection('Price overextended above EMA', {
      priceDistanceFromEMA: priceDistanceFromEMA,
      signalType: signalType,
      reason: 'Price too far above EMA - high probability of pullback'
    });
    return null;
  }
  ...
  // ============================================================
  // MANDATORY: Only enter trades in pullback zone (10-65% retracement)
  ...
  if (pullbackIdeal) {
    entryTimingBonus = 10; // Full bonus for ideal pullback (10-55%)
  } else if (inPullback) {

...
serve(async (req) => {
  ...
  try {
    ...
    const { data: positions } = await supabase.from("positions").select("*, trades(*)").eq("status", "active");
    ...
    for (const position of positions) {
      ...
      if (userSettings.enabled && pnlPercent > userSettings.activationPercent) {
        ...
        if (trailingActivated) {
          await supabase.from("positions").update({ stop_loss: newStopLoss }).eq("id", position.id);

          // Also update the trade record
          await supabase.from("trades").update({ stop_loss: newStopLoss }).eq("id", position.trade_id);

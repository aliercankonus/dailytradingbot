...
serve(async (req) => {
  ...
  const symbolDataPromises = symbols.map(async (symbol) => {
    ...
    for (const position of positions) {
      ...
      // Score 75-84: tighten stop + partial close (40% reduce)
      ...
      if (liquidityTrap?.detected && liquidityTrap.score >= 60 && !emergencyClose) {
        ...
        if (trapAgainstPosition) {
          if (trapScore >= 85) {
            ...
            // AGGRESSIVE: Tighten stop by ATR and mark for partial close
            const tightenAmount = currentATR * 0.75; // 75% ATR tightening
            ...
            const tightenAmount = currentATR * 0.5; // 50% ATR tightening
            let tightenedStop = currentSL;
            if (position.side === 'BUY') {
              tightenedStop = Math.max(currentSL, currentPrice - tightenAmount);
            } else {

...
export const getMomentumScore = (momentum: any, adx: number = 0, adxRising: boolean = false, stochRsiData?: { k: number; d: number }, adxSlope?: number): number => {
  if (!momentum) {
    ...
    if (adx >= MOMENTUM_ADX_FLOOR_PARAMS.VERY_STRONG_ADX_THRESHOLD && (adxSlope === undefined || adxSlope > MOMENTUM_ADX_FLOOR_PARAMS.VERY_STRONG_MIN_SLOPE)) {
      return MOMENTUM_ADX_FLOOR_PARAMS.VERY_STRONG_FLOOR;
    }
    return 0;
  }
  ...
  if (state === "confirmed" && confirms) {
    ...
    if (adx >= MOMENTUM_ADX_FLOOR_PARAMS.CONTINUATION_ADX_STRONG && adxRising) {
      score = 10;
    } else if (adx >= MOMENTUM_ADX_FLOOR_PARAMS.CONTINUATION_ADX_DECENT) {
      score = 8;
    } else {
      score = 6;
    }
    ...
    score = adx >= MOMENTUM_ADX_FLOOR_PARAMS.CONTINUATION_ADX_STRONG ? 4 : 2;
  } else {
    score = 0;
  }
  ...
  if (adx >= MOMENTUM_ADX_FLOOR_PARAMS.VERY_STRONG_ADX_THRESHOLD && effectiveAdxSlope > MOMENTUM_ADX_FLOOR_PARAMS.VERY_STRONG_MIN_SLOPE) {
    score = Math.max(score, MOMENTUM_ADX_FLOOR_PARAMS.VERY_STRONG_FLOOR);
  } else if (adx >= MOMENTUM_ADX_FLOOR_PARAMS.STRONG_ADX_THRESHOLD && effectiveAdxSlope > MOMENTUM_ADX_FLOOR_PARAMS.STRONG_MIN_SLOPE) {
    score = Math.max(score, MOMENTUM_ADX_FLOOR_PARAMS.STRONG_FLOOR);
  }
...
export const checkEarlyIgnitionException = (
  ...
  return {
    isValid: true,
    positionSizeMultiplier: ADX_GATE.EARLY_IGNITION_EXCEPTION_MULTIPLIER,  // 0.70x
    reasons,
    checkDetails,
  };
};
...
export const detectEarlyIgnitionEntry = (
  ...
  // v1.1 REFINED: Use EARLY_IGNITION_ENTRY constants directly (no duplicate local P object)
  const P = EARLY_IGNITION_ENTRY;
...
export const deriveTradeDirection = (
  ...
  if (GATE_RELAXATION_FLAGS.DIRECTION_WEIGHTED) {
    ...
    const getMomentumDirectionHint = (): number => {
      ...
      if (macdHistogram > 0 || rsi > RSI_ZONE_THRESHOLDS.MILD_BULLISH) return 0.5;
      if (macdHistogram < 0 || rsi < RSI_ZONE_THRESHOLDS.MILD_BEARISH) return -0.5;
      return 0;
    };
...
// extractAtrPercent, extractAtr, extractPriceChange, extractTimeframeTrend,
// extractMomentumState, extractCurrentPrice, extractADXWithSource,
// extractStochRsiWithSource functions have been moved to market-feature-snapshot.ts
// as private (non-exported) functions — they are only needed by the MFS builder.
// All downstream logic reads from MarketFeatureSnapshot fields directly.
